 ...
 * responsibility to open an input text file, to read its content, to apply a number
 * of transformations (via filters) and to write the result in an output text file.
 *
...
public abstract class FileTransformer implements IFileVisitor {
    ...
    * 0, 1 or more filter writers around the original writer and returns the result.
    *
  ...
  public void visit(File file) {
    ...
    try {
      ...
      writer = decorateWithFilters(writer);
         ...
         while ( (readBytes = reader.read(cbuff)) != -1 ) {
